...
describe('Renders a task correctly', () => {
  ...
  it('render the text given in props', () => {
    const mockTask = { task: 'Complete homework', complete: false };
    render(<Task task={mockTask.task} complete={mockTask.complete} />);
    expect(screen.getByText(mockTask.task)).not.toBeNull();
  });
  it('An incomplete task does not include "complete" in className list', () => {
    const mockTask1 = { task: 'Complete homework', complete: false };
    render(<Task task={mockTask1.task} complete={mockTask1.complete} />);
  ...
  it('A complete task includes "complete" in className list', () => {
    const mockTask2 = { task: 'Clean the house', complete: true };
    render(<Task task={mockTask2.task} complete={mockTask2.complete} />);
